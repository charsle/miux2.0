					...
					<table class="table table-striped fs12">
						...
						<tbody id="tableALLlist">
							<tr class="addPersons" v-for="newItem in newItems">
								...
								<td><input type="text" class="form-control" name="userPosintion" style="width: 80%;" maxlength="6"></td>
...
<script type="text/javascript">
	...
	export default {
		...
		computed: {
			...
			allChecked: {
				get: function() {
					return this.checkedCount == this.getMangeList.length;
				},
				set: function(value) {
					if (value) {
						this.checkedNames = this.getMangeList.map(function(item) {
							return item.TM00301;
						})
					} else {
						this.checkedNames = []
		...
		methods: {
			...
			importPersons() {
				gloabl.layer.open({
					...
					skin: '',
					btn: ['确定', '取消'],
					content: '<div style="padding:15px;">选择文件:<input type="file" id="xlsfile" name="file"><p class="mt10"><a href="../../../../static/resource/template.xls">下载标准模板</a>&nbsp;&nbsp;&nbsp;<span>用我们提供的标准模板编辑联系人信息</span></p></div>',
					yes() {

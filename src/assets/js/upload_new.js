...
var UploadFile = (function() {
     ...
     * @memberOf UploadFile
     */
    UploadFile.prototype.initFileUpload = function(callback) {
                        ...
                        if (_that.token !== '' || _that.token !== undefined) {
                            _that.checkFiles(chuckSum_1, function(data) {
                                if (data.code == 1) {
                                    ...
                                    _that.allUploadBlock(filedata_1, callback);
                                }
                            });
                        } else {
                            //如果没有获取到，继续去执行
     ...
     * @memberOf UploadFile
     */
    UploadFile.prototype.allUploadBlock = function(parmas, callback) {
        var _this = this;
        console.log(parmas);
        if (parmas.fileSize <= this.chukSize) {
            this.mkblk(0, parmas.fileSize, parmas.fileData.subarray(0, parmas.fileSize), parmas.token, function(res) {
                _this.ctxList.push(res.id);
                parmas.ctxList = _this.ctxList;
                _this.mkfile(parmas, callback);
            });
        } else if (parmas.fileSize <= this.blockSize) {
            this.mkblk(this.offset, parmas.fileSize, parmas.fileData.subarray(this.offset, (this.offset + this.chukSize)), parmas.token, function(res) {
                _this.ctxList.push(res.id);
                _this.offset += _this.chukSize;
                var flag = true,
                    vvtimer = null;
                vvbput();
                ...
                function vvbput() {
                    var _this = this;
                    if (this.offset < parmas.fileSize) {
                        var pSzie = parmas.fileSize - this.offset;
                        if (pSzie > this.chukSize) {
                            pSzie = this.chukSize;
                        }
                        this.bput(parmas.token, res.id, this.offset, parmas.fileData.subarray(this.offset, (this.offset + pSzie)), function(res) {
                            _this.offset += pSzie;
                            if (res.data.id) {
                                flag = true;
                                vvtimer = setTimeout(vvbput, 10);
                            } else {
                                flag = false;
                            }
                        });
                    } else if (this.offset == parmas.fileSize) {
                        parmas.ctxList = this.ctxList;
                        this.mkfile(parmas, callback);
                    } else {
                        console.log('错误');
                    }
                }
            ...
            var sendB = (function(that, parmas) {
                ...
                function sendBloack() {
                    ...
                    that.mkblk(0, blkSize, parmas.fileData.subarray(that.offset, (that.offset + pSize)), parmas.token, function(res) {
                        ...
                        timer = setTimeout(sendBloack, 10);
                    });
                    // } while (true)
                }
